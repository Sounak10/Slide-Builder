...
export type AgentTrackControlProps = VariantProps<typeof toggleVariants> & {
  ...
  /**
   * Props returned by LiveKit's useTrackToggle hook for controlling the real room track.
   */
  toggleButtonProps?: Omit<ComponentProps<'button'>, 'value'>;
  /**
   * Callback when a media device error occurs.
   */
  onMediaDeviceError?: (error: Error) => void;
  ...
  return (
    ...
    >
      <AgentTrackToggle
        ...
        pressed={pressed}
        pending={pending}
        disabled={disabled ?? toggleButtonDisabled}
        onPressedChange={toggleButtonProps ? undefined : onPressedChange}
        className={cn(
